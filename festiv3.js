...
const run=()=>{
  ...
  // Dates en FR (Notion)
  document.querySelectorAll(".notion-property-date,.notion-callout-text .notion-text").forEach(el=>{
    const m=el.textContent.match(/([A-Za-z]+ \d{1,2}, \d{4})( \d{2}:\d{2})?/);
    if(!m) return;
    const d=new Date(m[1]+(m[2]||""));
    if(isNaN(d)) return;
    const dateFR=d.toLocaleDateString(L,{day:"numeric",month:"long",year:"numeric"});
    if(m[2]){
      const hh=String(d.getHours()).padStart(2,"0");
      const mm=String(d.getMinutes()).padStart(2,"0");
      const day=d.toLocaleDateString(L,{weekday:"long"});
      el.textContent=`⏰ ${day} ${dateFR} dès ${hh}h${mm}`;
    } else el.textContent=dateFR;
  });

  // Footer 3 colonnes + copyright
  const footer=document.querySelector("footer.styles_main_footer__LoNow")||document.querySelector("footer");
